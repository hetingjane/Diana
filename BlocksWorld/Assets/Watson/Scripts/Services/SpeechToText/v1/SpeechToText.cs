    ...
    public class SpeechToText : IWatsonService
    {
        ...
        private bool CreateListenConnector()
        {
            if (_listenSocket == null)
            {
                ...
                if (_listenSocket == null)
                {
                	Log.Error("SpeechToText.CreateListenConnector()", "Unabled to create _listenSocket.");
                    return false;
                }
                else
                {
            ...
            if (msg is WSConnector.TextMessage)
            {
                ...
                if (json != null)
                {
                    if (json.Contains("results"))
                    {
                        ...
                        if (results != null)
                        {
                        	if (results.result_index > 0) {						// JJS
                        		Log.Debug("SpeechToText.OnListenMessage()", 
	                        		"Ignoring result with result_index={0}\n{1}", results.result_index, tm.Text);
	                        	return;
                        	}
        ...
        private SpeechRecognitionEvent ParseRecognizeResponse(IDictionary resp)
        {
            ...
            try
            {
	            ...
	            var srEvent = new SpeechRecognitionEvent(results.ToArray());
	            Log.Debug("FOO", "result_index=" + resp["result_index"]);
	            int dammit;
	            if (int.TryParse(resp["result_index"].ToString(), out dammit)) srEvent.result_index = dammit;
	            return srEvent;
            }
            catch (Exception e)
            {
