...
public class DianaEmotion : ImageResultsListener
{
    ...
    DataStore.IntValue joy;
    DataStore.IntValue sadness;
    ...
    public override void onImageResults(Dictionary<int, Face> faces)
    {
        if (faces.Count > 0)
        {
            ...
            Debug.Log(currentJoy);
            joy = new DataStore.IntValue((int)currentJoy);
            sadness = new DataStore.IntValue((int)currentSadness);
            moduleBase = new ModuleBase();

            DataStore.SetValue("user:joy:", joy, moduleBase, currentJoy.ToString());
            DataStore.SetValue("user:sadness:", joy, moduleBase, currentSadness.ToString());
