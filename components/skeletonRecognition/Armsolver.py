...
class Solver(object):
    def __init__(self, pointing_mode, lstm=False):
        ...
        self._class_list = ['emblems', 'motions', 'oscillate', 'one more', 'still']#np.load(self._logpath+'labels_list.npy')
...
class ArmMotionRecogntion(Solver):
    ...
    def arm_motion_direction(self, data, body_part):
        if (body_part == self._RIGHT) or (body_part == self._LEFT):
            ...
            probabilities = [probabilities[i] for i in [2, 3, 0, 1, 5, 4, 6]]
            #Removing still from probabilities
            if direction == 'still':
                probabilities = [0] * 6
            else:
