...
class BaseClassifier:
    ...
    def get_bytes(self, timestamp, writer_data_hand, engaged, frame_pieces, stream_id, probs, feature, blind, frame):
        self.probs = [0 for i in range(len(self.gestures))]


        max_index = self._process(feature, writer_data_hand, engaged, frame_pieces, probs, blind, frame)

        print('{:<20}'.format(self.gestures[max_index]), '{:.1}'.format(float(self.probs[max_index])), end='\t')
