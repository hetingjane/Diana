                    ...
                    if len(inputs) == streams.get_stream_count() + 1:
                        ...
                        if self.synced:
                            ...
                            if len(self.data_received[sync_ts]) == streams.get_stream_count():
                                ...
                                gui_data = list(s_data[1][4:])+list(s_data[2][3:])+list(s_data[4][3:])
                                print s_data[1]

                                output_data.put(struct.pack("!"+"f"*len(gui_data), *gui_data))


                                self.data_received.pop(sync_ts)


                                print len(gui_data), output_data.qsize()
