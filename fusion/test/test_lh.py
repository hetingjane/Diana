...
import time
import sys
from conf.postures import left_hand_postures
import conf.streams as streams

def process_frame(decoded_frame):
    time.sleep(0.03)
    return [streams.get_stream_id("LH"), fd[0], random.randint(0, len(left_hand_postures) - 1)] + [random.random() for x in range(len(left_hand_postures))]
...
if __name__ == '__main__':
    ...
    while True:
        ...
        sock_dest.sendall(struct.pack("!iqi" + "f" * len(left_hand_postures), *result))
